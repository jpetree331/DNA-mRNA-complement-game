          ...
          {selectedTeacher && (
            <div className="bg-slate-900/50 p-4 rounded-lg">
              ...
              {selectedAttempts.length === 0 ? (
                <p className="text-slate-300">No attempts recorded for this teacher.</p>
              ) : (
                <div className="space-y-4">
                  {selectedAttempts.map((attempt) => (
                    <div key={attempt.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <span className="text-cyan-400 font-bold">{attempt.userFirstName}</span>
                          <span className="text-slate-400 ml-2">Level {attempt.level}</span>
